...
def remove_airport() -> None:
    ...
    markers.remove(airport.marker)  # Remove the marker from the list
    airports.pop(i)
    show_airports()
...
def edit_airport_data() -> None:
    i = listbox_lista_obiektow.index(ACTIVE)
    airport = airports[i]
    entry_name.delete(0, END)
    entry_name.insert(0, airport.name)
    entry_location.delete(0, END)
    entry_location.insert(0, airport.location)
    button_update_airport.config(state=NORMAL)
    button_add_airport.config(state=DISABLED)
...
def remove_employee() -> None:
    ...
    markers.remove(employee.marker)  # Remove the marker from the list
    employees.pop(i)
...
def remove_customer() -> None:
    ...
    markers.remove(customer.marker)  # Remove the marker from the list
    customers.pop(i)
    show_employees_and_customers()
...
label_location = Label(frame, text="Location")
label_location.grid(row=1, column=0)
entry_location = Entry(frame)
entry_location.grid(row=1, column=1)

button_add_airport = Button(frame, text="Add Airport", command=add_airport)
button_add_airport.grid(row=2, column=0)
button_update_airport = Button(frame, text="Update Airport", command=update_airport)
button_update_airport.grid(row=2, column=1)
button_update_airport.config(state=DISABLED)
button_remove_airport = Button(frame, text="Remove Airport", command=remove_airport)
button_remove_airport.grid(row=2, column=2)
button_deselect_airport = Button(frame, text="Deselect Airport", command=deselect_airport)
button_deselect_airport.grid(row=2, column=3)

label_employee_name = Label(frame, text="Employee Name")
label_employee_name.grid(row=3, column=0)
entry_employee_name = Entry(frame)
entry_employee_name.grid(row=3, column=1)

label_employee_surname = Label(frame, text="Employee Surname")
label_employee_surname.grid(row=4, column=0)
entry_employee_surname = Entry(frame)
entry_employee_surname.grid(row=4, column=1)

label_employee_position = Label(frame, text="Employee Position")
...
label_employee_airport_id = Label(frame, text="Airport ID")
...
label_employee_location = Label(frame, text="Employee Location")
...
button_add_employee = Button(frame, text="Add Employee", command=add_employee)
button_add_employee.grid(row=8, column=0)
button_remove_employee = Button(frame, text="Remove Employee", command=remove_employee)
button_remove_employee.grid(row=8, column=1)

label_customer_name = Label(frame, text="Customer Name")
label_customer_name.grid(row=9, column=0)
entry_customer_name = Entry(frame)
entry_customer_name.grid(row=9, column=1)

label_customer_surname = Label(frame, text="Customer Surname")
...
label_customer_airport_id = Label(frame, text="Airport ID")
...
label_customer_location = Label(frame, text="Customer Location")
...
button_add_customer = Button(frame, text="Add Customer", command=add_customer)
button_add_customer.grid(row=13, column=0)
button_remove_customer = Button(frame, text="Remove Customer", command=remove_customer)
button_remove_customer.grid(row=13, column=1)

button_show_employees_and_customers = Button(frame, text="Show Employees and Customers", command=show_employees_and_customers)
...
label_name_szczegoly_obiektu = Label(root, text="Name: ")
...
label_location_szczegoly_obiektu = Label(root, text="Location: ")
...
# Notebook to show employees and customers within the main window
